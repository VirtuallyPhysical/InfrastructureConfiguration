...
# Import list of VMs from CSV
$vmList = Import-Csv -Path "C:\Path\To\vm_list.csv"  # Must have a column 'VMName'

# Define the DNS configuration script that will run inside each VM
$scriptText = @'
# =======================
...
foreach ($vmEntry in $vmList) {
    ...
    try {
        ...
        Write-Warning "[$vmName] Script execution failed: $_"
    }
}
